...
def _cmd_prompt(args: argparse.Namespace) -> int:
    from .prompt_to_canvas import prompt_to_canvas  # ліниво: тягне anthropic

    spec = prompt_to_canvas(args.prompt)
    if args.spec_out:
    ...
    with open(args.output, "wb") as fh:
        fh.write(pdf)
    print(f"OK: prompt -> {args.output} ({len(pdf)} байт)")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_prompt = sub.add_parser("prompt", help="prompt -> Canvas JSON -> print.pdf")
    p_prompt.add_argument("prompt", help="Текстовий запит")
